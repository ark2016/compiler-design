...
begin
  ...
  if (('a' <= CurrentChar) and (CurrentChar <= 'z')) or
     (('A' <= CurrentChar) and (CurrentChar <= 'Z')) then
  begin
    k := 0;
    ...
    while s <= SymPROC do
    begin
      if StringCompare(Keywords[s], CurrentIdentifer) then
        CurrentSymbol := s;
      s := s + 1;
    end;
  end
  else if (('0' <= CurrentChar) and (CurrentChar <= '9')) or (CurrentChar = '$') then
  begin
  ...
  else if (CurrentChar = '''') or (CurrentChar = '#') then
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if CurrentChar = '''' then
          begin
            if CurrentStringLength = MaximalStringLength then
              Error(147);
        ...
        else if (CurrentChar = #13) or (CurrentChar = #10) then
        begin
          Error(100);
          StrEnd := true;
        end
  ...
  begin
    ...
    begin
      ReadChar;
      LastChar := '-';
      while (CurrentChar <> #0) and not (((CurrentChar = ')') and (LastChar = '*')) or (CurrentChar = '}')) do
      begin
  ...
  begin
    ReadChar;
    { Для комментариев, начинающихся с '{', завершаем комментарий последовательностью "*)" }
    ...
    if CurrentChar <> #0 then
      ReadChar; { пропускаем завершающую ')' }
    GetSymbol;
  end
  else if CurrentChar = '/' then
  begin
